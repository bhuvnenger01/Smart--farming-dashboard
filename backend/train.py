...
def train_and_evaluate(run_id, retrain=False, prev_model=None, prev_lstm=None):
    ...
    # Use actual yield data if available, otherwise generate mock data
    ...
    # Generate and save fertilizer data (N, P, K deficits)
    fertilizer_data = torch.tensor(np.clip(100 - data.x[:, :3].numpy(), 0, 100), dtype=torch.float)
    with open(f'models/fertilizer_data_run{run_id}.pkl', 'wb') as f:
        pickle.dump(fertilizer_data, f)
